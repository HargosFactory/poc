[Return back](../readme.md)
<p align="center">
  <a href="https://vuejs.org/" target="blank"><img src="data:image/png;base64," width="300" alt="Vue Logo" /></a>
</p>

# [Explication code source ↲](todo)

## Introduction à Vue.js avec TypeScript et la Composition API

Vue.js est un framework JavaScript progressif utilisé pour construire des interfaces utilisateur. Avec Vue 3, la Composition API permet de structurer les composants de manière plus modulaire et réutilisable, et elle est particulièrement bien adaptée à TypeScript. Cette documentation explique comment utiliser Vue.js avec TypeScript en utilisant script setup.

### Configuration Initiale

Assurez-vous d'installer Vue.js avec TypeScript via Vue CLI :

```bash
vue create my-project
```

### Sélectionnez TypeScript lors de la configuration

Exemple de Composant Simple avec script setup et TypeScript
Le script setup simplifie la syntaxe pour l'utilisation de la Composition API dans les composants Vue. Voici un exemple simple pour comprendre les bases.

#### 1. Template HTML

Le template HTML contient la structure de votre composant, y compris l'utilisation de composants enfants et de balises HTML standard.

```html
<template>
  <div>
    <h1>{{ title }}</h1>
    <p>{{ description }}</p>
    <input v-model="title" placeholder="Edit the title"/>
    <button @click="incrementCount">Count is: {{ count }}</button>
  </div>
</template>
```

#### 2. Script Setup

Le script setup permet de définir la logique de votre composant de manière concise et claire.

```typescript
<script setup lang="ts">
import { ref } from 'vue';

const title = ref<string>('My Title');
const description = ref<string>('This is a description');
const count = ref<number>(0);

function incrementCount() {
  count.value++;
}
</script>
```

#### 3. Style Scoped

Le style scoped permet de définir des styles qui s'appliquent uniquement à ce composant.

```css
<style scoped>
h1 {
  color: blue;
}
</style>
```

### 4. Explication du Code

- Template HTML :

Utilisation des balises HTML standard pour afficher le contenu.
Utilisation de {{ }} pour afficher les valeurs réactives.
Liaison de l'entrée avec v-model pour l'édition de title.
Bouton avec un événement @click pour incrémenter le compteur.

- Script Setup :

Importation de ref de Vue pour définir des valeurs réactives.
Définition des valeurs réactives title, description, et count.
Définition de la fonction incrementCount pour incrémenter count.

- Style Scoped :

Définition des styles spécifiques au composant, avec scoped pour s'assurer qu'ils n'affectent pas d'autres parties de l'application.

### Avantages de la Composition API et script setup

- Modularité : Permet de mieux organiser le code par fonctionnalités.
- Lisibilité : Le script setup offre une syntaxe plus concise et simplifiée.
- TypeScript : Intégration native avec TypeScript pour une meilleure sécurité de type et une meilleure autocomplétion.
- Réutilisabilité : Les fonctions et les valeurs réactives peuvent être facilement réutilisées dans d'autres composants.

Ce guide fournit une base pour commencer avec Vue.js en utilisant TypeScript et la Composition API. Vous pouvez étendre ces concepts pour construire des composants plus complexes et des applications complètes.

## Commandes de base

```sh
$ npm install

$ npm run dev

$ npm run build
```
